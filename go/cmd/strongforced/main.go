...
func appExporter() server.AppExporter {
	return func(logger log.Logger, db db.DB, _ io.Writer, _ int64, _ bool, _ []string) (json.RawMessage, []tendermintTypes.GenesisValidator, error) {
		dapp := app.NewStrongForceApp(logger, db)
...
func InitCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if err = gaiaInit.ExportGenesisFile(genFile, chainID, []tendermintTypes.GenesisValidator{validator}, appState); err != nil {
				return err
			}

			tendermintConfig.WriteConfigFile(filepath.Join(config.RootDir, "config", "config.toml"), config)

			fmt.Printf("Initialized strongforced configuration and bootstrapping files in %s...\n", viper.GetString(cli.HomeFlag))
			return nil
		},
	}
...
$ strongforced add-genesis-account cosmos1tse7r2fadvlrrgau3pa0ss7cqh55wrv6y9alwh 1000STAKE,1000nametoken
`),
		RunE: func(_ *cobra.Command, args []string) error {
			...
			var genDoc tendermintTypes.GenesisDoc
			config := ctx.Config
			genFile := config.GenesisFile()
...
func SimpleAppGenTx(cdc *codec.Codec, pk crypto.PubKey) (
	appGenTx, cliPrint json.RawMessage, validator tendermintTypes.GenesisValidator, err error) {
	...
	bz, err := cdc.MarshalJSON(struct {
		Addr types.AccAddress `json:"addr"`
	}{addr})
	if err != nil {
		return
	}
	...
	validator = tendermintTypes.GenesisValidator{
		PubKey: pk,
		Power:  10,
	}
