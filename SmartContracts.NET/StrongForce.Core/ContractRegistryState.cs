...
{
	public class ContractRegistryState : IStatefulObject
	{
		public ContractRegistryState(IDictionary<string, object> state)
		{
			if (state != null)
			{
				this.SetState(state);
		...
		private void SetState(IDictionary<string, object> state)
		{
			...
			this.AddressFactory = BaseAddressFactory.Create(factoryType, factoryState);
